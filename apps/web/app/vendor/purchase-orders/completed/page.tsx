  ...
  const fetchPurchaseOrders = async (token?: string) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        const completedOrders = data.data.filter((po: PurchaseOrder) => 
          po.status === 'completed'
        )
    ...
    <VendorLayout>
        ...
        <div className="flex justify-between items-center">
          <div>
            <div className="flex items-center space-x-2 mb-1">
              ...
              <h1 className="text-2xl font-bold text-gray-900">Completed Purchase Orders</h1>
            </div>
            <p className="text-gray-600 mt-1">Orders that have been successfully delivered</p>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div className="bg-white rounded-xl shadow-sm p-4 border border-gray-100">
          ...
          <p className="text-xl font-bold text-green-600">{purchaseOrders.length}</p>
        </div>
        <div className="bg-white rounded-xl shadow-sm p-4 border border-gray-100">
        ...
        <div className="flex flex-col md:flex-row gap-3">
          <div className="flex-1 relative">
            ...
            <input
              ...
              className="w-full h-10 pl-9 pr-3 border border-gray-300 rounded-lg text-sm focus:outline-none focus:border-green-500 focus:ring-1 focus:ring-green-500"
            />
          </div>
            ...
            <tbody className="divide-y divide-gray-200">
              {currentItems.map((po) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 whitespace-nowrap text-gray-600">{formatDate(po.deliveredDate)}</td>
              ...
              {currentItems.length === 0 && (
                <tr>
                  <td colSpan={8} className="px-6 py-12 text-center text-gray-500">
                    ...
                    <p>No completed purchase orders found</p>
                  </td>
                </tr>
              )}
            </tbody>
      ...
      {showDetails && selectedPO && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-xl max-w-7xl w-full max-h-[90vh] overflow-y-auto">
            <div className="px-6 py-4 border-b flex justify-between items-center bg-green-600 text-white rounded-t-xl sticky top-0">
              <h3 className="text-lg font-semibold">PO Details: {selectedPO.poNumber}</h3>
              <button onClick={closeDetails} className="p-1 hover:bg-green-700 rounded-lg">
                <X size={20} />
              </button>
            </div>
            <div className="p-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                ...
                <div className="bg-gray-50 p-3 rounded-lg">
                  ...
                  <p className="text-sm font-semibold">{formatDate(selectedPO.deliveredDate)}</p>
                </div>
              </div>
