...
export default function VendorDashboard() {
  ...
  const fetchPurchaseOrders = async (token?: string) => {
    try {
      ...
      if (!response.ok) {
        console.log('⚠️ Could not fetch POs, showing empty state')
        // Set empty stats
        setStats({
          totalPOs: 0,
          pendingPOs: 0,
      ...
      if (data.success && data.data) {
        ...
        const pendingPOs = pos.filter((po: any) => po.status === 'pending' || po.status === 'draft').length
        const completedPOs = pos.filter((po: any) => po.status === 'completed').length
        const totalItems = pos.reduce((sum: number, po: any) => sum + (po.lineItems?.length || 0), 0)
