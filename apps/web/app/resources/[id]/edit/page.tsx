...
export default function EditResourcePage() {
    ...
    try {
      ...
      if (response.success) {
        ...
        setFormData({
          ...
          type: response.data.type || 'equipment',
          ...
          lastMaintenance: formatDate(response.data.lastMaintenance),
          nextMaintenance: formatDate(response.data.nextMaintenance),
          maintenanceInterval: response.data.maintenanceInterval?.toString() || '',
          hourlyRate: response.data.hourlyRate?.toString() || '',
          dailyRate: response.data.dailyRate?.toString() || '',
          purchaseCost: response.data.purchaseCost?.toString() || ''
        })
      } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await api.updateResource(params.id as string, resourceData)
      
      if (response.success) {
        router.push(`/resources/${params.id}`)
      } else {
        setError(response.error || 'Failed to update resource')
      }
    } catch (err: any) {
