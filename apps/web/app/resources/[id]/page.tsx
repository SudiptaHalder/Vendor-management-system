 ...
 const handleDeleteResource = async () => {
  ...
  const getAssignmentStatusBadge = (status: string) => {
    const colors: Record<string, string> = {
      ...
      ended: 'bg-gray-100 dark:bg-gray-800 text-gray-800 dark:text-gray-200'
    }
    return (
    ...
    return (
      <MainLayout>
        <div className="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-100 dark:border-gray-800 p-12 text-center">
          ...
          <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-2">Error Loading Resource</h3>
          <p className="text-gray-500 dark:text-gray-400 mb-6">{error || 'Resource not found'}</p>
          <Link
            href="/resources"
    ...
    <MainLayout>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        ...
        <div className="lg:col-span-2 space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              {resource.type && (
                <div>
                  ...
                  <p className="text-sm font-medium text-gray-900 dark:text-gray-100 capitalize">{resource.type}</p>
                </div>
              )}
              {resource.category && (
                <div>
                  <p className="text-xs text-gray-500 dark:text-gray-400">Category</p>
                  <p className="text-sm font-medium text-gray-900 dark:text-gray-100">{resource.category}</p>
                </div>
              )}
              {resource.model && (
                <div>
                  <p className="text-xs text-gray-500 dark:text-gray-400">Model</p>
                  <p className="text-sm font-medium text-gray-900 dark:text-gray-100">{resource.model}</p>
                </div>
              )}
              {resource.serialNumber && (
                <div>
                  <p className="text-xs text-gray-500 dark:text-gray-400">Serial Number</p>
                  <p className="text-sm font-medium text-gray-900 dark:text-gray-100">{resource.serialNumber}</p>
                </div>
              )}
              {resource.capacity && (
                <div>
                  <p className="text-xs text-gray-500 dark:text-gray-400">Capacity</p>
                  <p className="text-sm font-medium text-gray-900 dark:text-gray-100">{resource.capacity}</p>
                </div>
              )}
              {resource.location && (
                <div>
                  <p className="text-xs text-gray-500 dark:text-gray-400">Location</p>
                  <p className="text-sm font-medium text-gray-900 dark:text-gray-100">{resource.location}</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-100 dark:border-gray-800 overflow-hidden">
            <div className="px-6 py-4 border-b bg-gray-50 dark:bg-gray-800 flex items-center justify-between">
              <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100">Current Assignments</h2>
              <Link
                href={`/resources/${resource.id}/assign`}
            ...
            {resource.assignments.length === 0 ? (
              ...
              <div className="divide-y divide-gray-100 dark:divide-gray-800">
                {resource.assignments.filter(a => a.status === 'active').map((assignment) => (
                  <div key={assignment.id} className="p-6 hover:bg-gray-50 dark:hover:bg-gray-800">
                    <div className="flex items-start justify-between mb-3">
                      <div>
                        ...
                        {assignment.notes && (
                          <p className="text-sm text-gray-500 dark:text-gray-400 mt-2">{assignment.notes}</p>
                        )}
                      </div>
        ...
        <div className="space-y-6">
          {/* Status Card */}
          <div className="bg-white dark:bg-gray-900 rounded-xl shadow-sm p-6 border border-gray-100 dark:border-gray-800">
            <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-4">Status & Availability</h2>
            <div className="space-y-4">
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="text-sm font-medium text-gray-900 dark:text-gray-100">{resource.assignments.length}</span>
              </div>
              <div className="flex justify-between items-center">
            ...
            <div className="space-y-3">
              {resource.hourlyRate && (
                <div className="flex justify-between">
                  ...
                  <span className="text-sm font-medium text-gray-900 dark:text-gray-100">${resource.hourlyRate}/hr</span>
                </div>
              )}
              {resource.dailyRate && (
                <div className="flex justify-between">
                  <span className="text-sm text-gray-600 dark:text-gray-400">Daily Rate</span>
                  <span className="text-sm font-medium text-gray-900 dark:text-gray-100">${resource.dailyRate}/day</span>
                </div>
              )}
              {resource.purchaseCost && (
                <div className="flex justify-between">
                  <span className="text-sm text-gray-600 dark:text-gray-400">Purchase Cost</span>
                  <span className="text-sm font-medium text-gray-900 dark:text-gray-100">${resource.purchaseCost.toLocaleString()}</span>
                </div>
              )}
            </div>
          </div>
            ...
            <div className="space-y-3">
              ...
              {resource.maintenanceInterval && (
                <div className="flex justify-between">
                  ...
                  <span className="text-sm font-medium text-gray-900 dark:text-gray-100">{resource.maintenanceInterval} days</span>
                </div>
              )}
            </div>
          </div>
          ...
          {/* Metadata */}
          <div className="bg-white dark:bg-gray-900 rounded-xl shadow-sm p-6 border border-gray-100 dark:border-gray-800">
            <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-4">Resource Info</h2>
            <div className="space-y-3">
              <div className="flex justify-between">
                <span className="text-xs text-gray-500 dark:text-gray-400">Created By</span>
                <span className="text-sm text-gray-900 dark:text-gray-100">{resource.createdBy.name || resource.createdBy.email}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-xs text-gray-500 dark:text-gray-400">Created On</span>
                <span className="text-sm text-gray-900 dark:text-gray-100">{new Date(resource.createdAt).toLocaleDateString()}</span>
              </div>
            </div>
          </div>
        </div>
