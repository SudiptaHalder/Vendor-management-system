...
async getResources(params?: { 
  ...
  status?: string, 
  projectId?: string,
  search?: string  // ✅ ADD THIS - for searching by name, serial, model
}) {
...
async assignResource(data: any) {
  const response = await fetch(`${API_URL}/resource-assignments/assignments`, { // ✅ FIXED: added /assignments
    method: 'POST',
    headers: getHeaders(),
    body: JSON.stringify(data)
  })
...
async updateResourceAssignment(id: string, data: any) {
  const response = await fetch(`${API_URL}/resource-assignments/assignments/${id}`, { // ✅ FIXED: added /assignments
    method: 'PUT',
    headers: getHeaders(),
    body: JSON.stringify(data)
  })
