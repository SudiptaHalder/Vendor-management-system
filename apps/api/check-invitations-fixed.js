// Use the database package directly
...
async function main() {
  ...
  for (const invite of invitations) {
    ...
    // Show temp password hint (first 4 chars)
    if (invite.tempPassword) {
      console.log(`Temp password hash starts with: ${invite.tempPassword.substring(0, 10)}...`)
    }
  }
  ...
  if (pending > 0) {
    console.log('\n⚠️  Emails not sent yet. Need to trigger email sending.')
  }
}
