  ...
  try {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      try {
        ...
        // Extract supplier code and name using actual headers
        const supplier = cleanValue(rowData['Supplier'])
        const nameOfSupplier = cleanValue(rowData['Name of Supplier'])

        if (!supplier || !nameOfSupplier) {
          summary.errors.push(`Row ${i + 2}: Missing Supplier or Name of Supplier`)
          continue
        }
        ...
        if (!vendor) {
          ...
          vendor = await prisma.vendors.create({
            data: {
              ...
              supplierName: nameOfSupplier,
              email: cleanValue(rowData['E-Mail Address']) || `${supplier.toLowerCase()}@vendorflow.com`,
              status: 'active'
            }
          })
          summary.created++
          console.log(`✅ Created new vendor: ${supplier}`)
        }
        ...
        // Map Excel columns to database fields based on actual headers
        ...
        if (existing) {
          ...
          summary.updated++
          console.log(`🔄 Updated master data for: ${supplier}`)
        } else {
          await prisma.vendorMaster.create({
            data: masterData
          })
          summary.created++
          console.log(`✅ Created master data for: ${supplier}`)
        }
